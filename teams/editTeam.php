...
$sql2 = "SELECT team_description, team_manager FROM team WHERE team_id = '{$team_id}'";
$result2 = mysqli_query($conn, $sql2);
if (mysqli_num_rows($result)) {
	$row = mysqli_fetch_assoc($result2);
		...
		<article class="rightArt" id="addTeam">
			...
			<tbody>
				<?php
					...
					$count = 1;
					$sql = "SELECT user_id, user_first_name, user_last_name, user_login FROM user WHERE visibility = 1 ORDER BY user_last_name";
					$result = mysqli_query($conn, $sql);
					if (mysqli_num_rows($result) > 0){
						while ($row=mysqli_fetch_assoc($result)){
							...
							$sql3 = "SELECT * FROM user_team WHERE team_id='{$team_id}' AND user_id = '{$user_team_id}' AND visibility = 1 ";
