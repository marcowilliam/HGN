	...
	<content>
		...
		<table id="tabela">
			...
			<form action="newteam.php" method="post" id="newTeam">
				<tr class="header">
					<th colspan=2><input type="text" id="team_name1" placeholder="Team Name" name="team_name"/></th>
			...
			<tbody>
				<?php
					...
					$count = 1;
					$sql = "SELECT team_id, team_description FROM team WHERE visibility = 1 ORDER BY team_description";
					$result = mysqli_query($conn, $sql);
					if (mysqli_num_rows($result) > 0){
						while ($row=mysqli_fetch_assoc($result)){
							...
							echo "</td> 
								<td class='td_icon' valign='middle' style='padding-right: 0px;'>
									"; ?>
									<i class="fa fa-pencil-square-o fa-lg smallIcon" onClick="window.location.href='editTeam.php?v=<?php echo $row["team_id"];?>'"></i>
								</td>
								<td class='td_icon'> <i class="fa fa-times fa-lg smallIcon" style="color: red" onClick="window.location.href='deleteTeam.php?v=<?php echo $row["team_id"];?>'"></i>  </td>
							</tr> <?php
						}
					}  else {
					
						echo "<tr><td style='text-align: center;'>No Visible Teams</td></tr>";
					
					} ?>
			</tbody>
		</table>
		
		<h1 style="font-weight: 300; font-size: 200%; padding-top: 10%;"> <i class="fa fa-times fa-lg" style="font-size: 100%; color: red;"></i> Deleted Teams </h1>
		...
		<table id="tabela2">
			...
			<tbody>
				<?php
					...
					$count = 1;
					$sql = "SELECT team_id, team_description FROM team WHERE visibility = 0 ORDER BY team_description";
					$result = mysqli_query($conn, $sql);
					if (mysqli_num_rows($result) > 0){
						while ($row=mysqli_fetch_assoc($result)){
							if (( $count % 2 )== 0){
							...
							echo "</td> 
								<td class='td_icon' valign='middle'>
									"; ?>
									<i class="fa fa-undo fa-lg smallIcon" onClick="window.location.href='recoverTeam.php?v=<?php echo $row["team_id"];?>'"></i>
								</td> 
							</tr> <?php
						}
					} else {
						echo "<tr><td style='text-align: center;'>No Deleted Teams</td></tr>";
					}
