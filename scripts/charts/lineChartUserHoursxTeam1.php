...
for($x = 0; $x<count($userId); $x++){
	...
	$sql4 = "SELECT task.task_id FROM task LEFT JOIN user_task ON user_task.task_id = task.task_id WHERE user_task.user_id = '{$userId[$x]}'";
...
for($x = 0; $x < count($userTasks); $x++){
	for($y = 0; $y < count($userTasks[$x]); $y++){
			$sql5 = "SELECT sub_task.sub_task_date, sub_task.sub_task_time FROM sub_task LEFT JOIN user_task ON sub_task.user_task_id = user_task.user_task_id WHERE user_task.task_id = '{$userTasks[$x][$y]}'";
